...
def main():
	...
	for subindex in range(0,len(subname[0])):
		gtPathFile.append(stor+directorylist[0]+'/Split-'+str(1)+'-'+str(1)+'/gt_'+txtlist[0]+'_test11'+subname[0][subindex]+'.txt')
	...
	for s in range(1,4): #2,3 -> macht nur 2
		for f in range(1,6): #1,7 -> macht 1-6
			for index in range(0,len(directorylist)):
				gtPathFile.append(stor+directorylist[index]+'/Split-'+str(s)+'-'+str(f)+'/gt_'+txtlist[index]+'_fold'+str(s)+str(f)+'valid.txt')
				folder_dir.append(stor+directorylist[index]+'/Split-'+str(s)+'-'+str(f)+'/results_'+evalname+'_'+txtlist[index]+'_fold'+str(s)+str(f)+'valid_best_Split_'+str(s)+'_'+str(f)+'.txt')
	print(gtPathFile)
	print(folder_dir)
	a = 'w+'
	mAP_list = []
