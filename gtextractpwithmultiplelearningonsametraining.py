...
labelPath =os.path.expanduser('~/Documents/darknet-yolov4/Master/DatasetsPink/')
evaluationname='lab7'
evalnamecaps='Lab7'
...
#the order of these has to match with the order of variants do not change it if you expand pink with new subvariants rgb needs to be expanded as well
subvariants=[['Tag','Abend','Nacht','Schwarz'],['Tag','Abend','Nacht','Schwarz'],['Tag'],['Tag'],['Tag']]
im_width = 570
im_height = 430
...
for si in range(1,s+1):    
        ...
        dataTXT.append(evaltxtpath+'/Pink/Split-'+str(si)+'/fold'+str(si)+'valid.txt')
        ...
        for subindex in range(0,len(subvariants[0])):
                dataTXT.append(evaltxtpath+'/Pink/Split-'+str(si)+'/'+evaluationname+'_'+variants[0]+'_test'+subvariants[0][subindex]+'.txt')
        for index in range(0,len(variants)):
             if index > 0:
                resultTXT.append([])
             #gt file for fold
             resultTXT[index].append(resultxtpath+'/'+variantpathnames[index]+'/Split-'+str(si)+'/gt_'+variants[index]+'_fold'+str(si)+'valid.txt')
             #if you want to do this only for pink set write 1 instead of len(variants)       
             for subindex in range(0,len(subvariants[index])):
                resultTXT[index].append(resultxtpath+'/'+variantpathnames[index]+'/Split-'+str(si)+'/gt_'+variants[index]+'_test'+str(si)+subvariants[index][subindex]+'.txt')
        ...
        for m in range(0,len(dataTXT)): 
            with open(dataTXT[m]) as evalFile: 
                for numEval, lineEval in enumerate(evalFile, 1):                
                    ...
                    if None != re.search(subvariants[0][3],filename):
                        nwfd=1                  
                        filename=re.sub(r'Schwarz','Tag',filename)
